    ...
    <div className="grid items-center  md:gap-16 lg:grid-cols-2">
      <div className="relative space-y-6">
        <div className="flex items-center gap-4">
          ...
          >
            <step.icon
              className={`h-8 w-8 ${selectedPath === 'hr' ? 'text-emerald-600' : 'text-gold-600'}`}
            />
          </div>
          <div
        ...
        <h2 className="text-3xl font-black text-gray-900 dark:text-gray-100 md:text-4xl lg:text-5xl">
          {t(step.titleKey)}
        </h2>
      ...
      <motion.div
        style={{ y: deviceY }}
        className="relative flex items-center justify-center md:mt-32"
      >
        <div className="relative w-full max-w-2xl">
          <div
            className={`flex aspect-[9/16] items-center justify-center overflow-visible ${scaleClass}`}
          >
            <Image
          ...
          {/* Rozeti görselin alt kenarına yasla (hafif overlap) */}
          <div className="pointer-events-none absolute bottom-32 left-1/2 -translate-x-1/2">
            <span
              className={`inline-flex items-center rounded-full border px-4 py-1.5 text-xs font-semibold shadow-sm ${
                selectedPath === 'hr'
